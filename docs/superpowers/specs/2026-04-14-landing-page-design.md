...
ヒーロー右側の 3 作品カード（月光回廊 / 静かな庭 / 時を渡る島）は**静的な装飾**。実 API からデータを取得せず、JSX にハードコードする。

**作品名は全て架空**。以下の 2 リスクを回避するため、実在作品は使わない:

1. **商標リスク**: 実在作品名（「葬送のフリーレン」等）は商標登録されている可能性があり、Recolly の宣伝素材として使うと商標権侵害のリスクがある。
2. **景品表示法リスク**: 実在作品に架空スコア「9.2 / 10」を紐付けると、Recolly のユーザー評価ではないにもかかわらず、あたかもそう見える。実ユーザーがゼロの初期段階では優良誤認に該当するリスクがある。

架空名は brainstorming 段階で決定した 3 つを使う:

| ジャンル | タイトル | 狙い |
|---|---|---|
| アニメ | 月光回廊 | 詩的だが派手すぎないトーン |
| 本 | 静かな庭 | 「静か」をブランドボイスと揃える |
| ゲーム | 時を渡る島 | 冒険感と静謐さの両立 |

架空作品なので、ジャンル色ドット・進捗バー・評価スコアは自由に設定できる。
