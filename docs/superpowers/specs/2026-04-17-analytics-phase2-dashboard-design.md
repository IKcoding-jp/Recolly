...
3. PostHog UI で「Recolly Main Dashboard」が作成されており、本仕様で定義した 8 つの Insight が表示されていることを確認する。
...
- `insights.ts`:
  - Insight 8 件の定義を TypeScript オブジェクトの配列として export。
...
| 3 | `Cross-genre User Rate` | Trends (user property filter) | User Property `distinct_media_types_count >= 2` の unique user 数 ÷ 全 identify 済み unique user 数 | **ジャンル横断率** — Recolly の差別化の核 |
...
- User Property: `signup_method`, `signup_date`（既存）、`distinct_media_types_count`（**新規**）
...
4. PostHog UI で「Recolly Main Dashboard」が作成され、8 つの Insight が正しく表示されているか確認。
