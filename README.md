...
Recollyは、実装のすべてをClaude Code（AIエージェント）が担当し、私はプロダクトの意思決定とレビューに専念する「バイブコーディング」という開発スタイルで作っています。

| 役割 | 担当 |
|------|------|
| 要件定義・プロダクトコンセプト | 私 |
| UI/UXの方向性判断 | 私 |
| 技術選定の意思決定（AIが提示した選択肢とトレードオフを理解した上で選定） | 私 |
| コードレビュー・受け入れ確認 | 私 |
| 実装コード（バックエンド・フロントエンド・インフラ全て） | Claude Code |

**実際に私が下した意思決定の例:**

- **プロダクトコンセプト** — 「物語が好きな人はジャンルを横断してコンテンツを楽しむ」という課題観察から、6ジャンル横断の記録アプリという企画自体を発案しました。
- **UI設計の方針** — 白背景・クリーン・モダンなデザインを指定し、AIが作りがちな「いかにもダークテーマ」を避ける方針を明確に指示しました。

実装コードは、Terraformのインフラ定義からRailsのモデル、Reactのコンポーネントまで、すべてClaude Codeが記述しています。**私自身はこのリポジトリのコードを一行も書いていません。** 現在は独習JavaScriptでプログラミングの基礎を学習中で（詳細は[関連プロジェクト](#関連プロジェクト)を参照）、今後はRecollyの実装にも自分のコードで貢献していく予定です。
...
技術選定は、Claude Codeが選択肢とトレードオフを提示し、私が理解した上で意思決定しました。判断の詳細は下記の[ADR](#設計判断の記録adr)に記録しています。
...
技術選定時には必ず「なぜその技術を選んだか」をADRに記録し、判断の背景を将来にわたって参照可能にしています。

---

## 設計判断の記録（ADR）

技術選定のたびに、Claude Codeが選択肢とトレードオフを提示し、私が理解した上で意思決定した内容をADR（Architecture Decision Record）として記録しています。「何を選んだか」だけでなく「なぜ選んだか」「他に何を検討したか」を残すことで、判断の背景を将来にわたって参照可能にしています。
...
Recollyの実装はAIが担当していますが、コーディング自体は別のプロジェクトで学習・実践しています。

- **独習JavaScript** — 書籍『独習JavaScript』を1章ずつ手を動かしてこなしているリポジトリ
- **ポモドーロタイマー** — 独習JSと並行して自作したタイマーアプリ
- **積みゲートラッカー** — React + JavaScriptで開発・公開中のアプリ

→ [GitHubプロフィール](https://github.com/IKcoding-jp)で公開中のリポジトリを確認できます。

現在はフロントエンドエンジニアとしての就職を目指して学習を進めており、独習JavaScript完走後はRecollyの実装にも自分のコードで貢献していく予定です。
